...
public class ToTalProductDAO {
    ...
    public List<Product> getAllProducts(int currentPage, int nuPerPage, String option) {
        ...
        String sql = """
    SELECT
        p.id, p.name, p.quantity, p.addedDate, p.description, 
        p.height, p.weight, p.width, p.selling, p.img,
        c.id AS category_id, c.name AS category_name,
        pr.id AS price_id, pr.price, pr.discountPercent, pr.lastPrice,
        ti.id AS technical_info_id, ti.specifications, ti.manufactureDate, ti.releaseDay
    FROM
        products p
    JOIN
        ...
        return jdbi.withHandle(handle ->
                handle.createQuery(sql)
                        ...
                        .map((rs, ctx) -> {
                            ...
                            technicalInfo.setReleaseDay(rs.getDate("releaseDay").toLocalDate());
