...
<div class="container my-5">
    <div class="row">
        ...
        <div class="col-md-3">
            ...
            <!-- 10.000đ - 20.000đ -->
            <div class="form-check mb-2">
                <input class="form-check-input price-filter" type="checkbox" id="price2" data-min="10000"
                       data-max="20000"
            ...
            <!-- 20.000đ - 50.000đ -->
            <div class="form-check mb-2">
                <input class="form-check-input price-filter" type="checkbox" id="price3" data-min="20000"
                       data-max="50000"
            ...
            <!-- 50.000đ - 100.000đ -->
            <div class="form-check mb-2">
                <input class="form-check-input price-filter" type="checkbox" id="price4" data-min="50000"
                       data-max="100000"
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        <li><a class="dropdown-item"
                               href="product-buttonUp?option=1&page=1<c:if test='${minPrice != null || maxPrice != null}'>&amp;minPrice=${minPrice}&amp;maxPrice=${maxPrice}</c:if>">Mới
                            nhất</a></li>
                        <li><a class="dropdown-item"
                               href="product-buttonUp?option=2&page=1<c:if test='${minPrice != null || maxPrice != null}'>&amp;minPrice=${minPrice}&amp;maxPrice=${maxPrice}</c:if>">Giá:
                            Cao -> Thấp</a></li>
                        <li><a class="dropdown-item"
                               href="product-buttonUp?option=3&page=1<c:if test='${minPrice != null || maxPrice != null}'>&amp;minPrice=${minPrice}&amp;maxPrice=${maxPrice}</c:if>">Giá:
                            Thấp -> Cao</a></li>
                        <li><a class="dropdown-item"
                               href="product-buttonUp?option=4&page=1<c:if test='${minPrice != null || maxPrice != null}'>&amp;minPrice=${minPrice}&amp;maxPrice=${maxPrice}</c:if>">Bán
                            chạy nhất</a></li>
                        <li><a class="dropdown-item"
                               href="product-buttonUp?option=5&page=1<c:if test='${minPrice != null || maxPrice != null}'>&amp;minPrice=${minPrice}&amp;maxPrice=${maxPrice}</c:if>">Giảm
                            giá: Cao -> Thấp</a></li>
                    </ul>
                </div>
            </div>
                  ...
                  <span class="badge bg-danger position-absolute top-0 end-0 m-2 px-3 py-2 fs-5 product-discount">
                    -<fmt:formatNumber value="${product.price.discountPercent}" pattern="##0"/>%
                            ...
                            <img id="mainImage${product.id}" src="${product.image}" alt="${product.description}"
                                 class="card-img-top main-product-image" style="object-fit: cover; cursor: pointer;">
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-body">
                                            <img src="${product.image}" class="img-fluid" id="modalImage${product.id}"
                                                 alt="Hình ảnh lớn">
                                        </div>
                      ...
                      <fmt:formatNumber value="${product.price.price}" type="currency" currencySymbol="₫"/>
                                ...
                                <p class="cart-text description"><fmt:message key="mota"/>: ${product.description}</p>

                                <!-- Form Thêm Vào Giỏ Hàng -->
                                <form action="cart?method=add" method="post" class="mt-auto product-options-form">
                                    <!-- Hidden Inputs -->
                                    <input type="hidden" name="currentURL"
                                           value="product-buttonUp?page=${currentPage}&option=${option}<c:if test='${minPrice != null || maxPrice != null}'> &amp;minPrice=${minPrice}&amp;maxPrice=${maxPrice}</c:if>">
                                    ...
                                    <div class="mb-3 quantity-container" style="display: none;">
                                        <label for="quantity${product.id}" class="form-label"><fmt:message
                                                key="soluong"/></label>
                                        <input type="number" class="form-control quantity-input"
                                               id="quantity${product.id}" name="quantity" min="1" value="1" required>
                                    </div>

                                    <!-- Nút Thêm Vào Giỏ Hàng / Xác Nhận -->
                                    <button type="button" class="btn btn-warning w-100 mb-2 add-to-cart-button">
                                        <fmt:message key="addCart"/>
                                    </button>
                                    <button type="submit" class="btn btn-success w-100 mb-2 submit-cart-button"
                                            style="display: none;"><fmt:message key="xacNhan"/>
                                    </button>
                                </form>

                                <!-- Nút Xem Ngay -->
                                <a href="detail-product?productId=${product.id}"
                                   class="btn btn-primary w-100"><fmt:message key="xem"/></a>
                            </div>
                        </div>
                    </div>
                </c:forEach>
